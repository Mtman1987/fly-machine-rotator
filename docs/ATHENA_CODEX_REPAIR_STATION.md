...
spmt-live:        CODEX_WORKER_SECRET (only when the SPMT deployment calls the worker directly)
mtman-machine-rotator: CODEX_WORKER_SECRET (optional stable override; generated locally by default)
...
Inside the Rotator/Fly server boundary, the CLI automatically reads the root-only credential generated by the running Rotator and uses `ATHENA_CODER_BASE_URL` (by default the private dashboard port, `PORT + 2`). This direct mode is intended for smoke tests and operations running on the server; it does not need a browser session, copied user token, or extra Fly flag.
