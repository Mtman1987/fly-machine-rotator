...
export function buildAthenaGatewayPayload(identity: SpmtIdentity, body: AthenaProxyChatRequest) {
  ...
  return {
    ...
    admin,
    payload: {
      // StreamWeaver derives tenant and authority again from the forwarded
      // SPMT token. These values are compatibility hints, not trusted claims.
      tenantId: identityId,
...
export async function handleAthenaOsChatProxy(
  ...
  if (!accessToken) return sendJson(response, 401, { error: "SPMT login required" });
  const identity = await requireSpmtIdentity(request, env);
  if (!identity) return sendJson(response, 401, { error: "SPMT session expired. Sign in again." });
  ...
  // Forward the SPMT-issued access token already present in the signed-in
