...
export async function handleLlmControlUiRequest(
  ...
  if (request.method === "GET" && url.pathname === "/llm-control") {
    html(response, renderPage(String(admin.username || admin.id || "SPMT admin")));
    return true;
  }
...
function renderPage(adminName: string) {
  const safeAdmin = adminName.replace(/[&<>"']/g, (character) => ({
    "&": "&amp;", "<": "&lt;", ">": "&gt;", "\"": "&quot;", "'": "&#39;",
  })[character] || character);
  ...
  <h1>SPMT Local LLM Control</h1><p class="muted">Authorized by SPMT as ${safeAdmin}. Model files remain on the worker's persistent Fly volume.</p>
