...
async function publishJob(job: PublicCodexJob, env: NodeJS.ProcessEnv) {
  ...
  const pushed = await pushRepoBranch(workspace, branch, `Stella: ${job.description.slice(0, 64)}`, env);
  ...
  const response = await fetch(`https://api.github.com/repos/${repository}/pulls`, {
    ...
    body: JSON.stringify({
      title: `Stella: ${job.description.slice(0, 72)}`,
      head: branch,
      base: "main",
      draft: true,
      body: `## What changed\n\n${job.summary || job.description}\n\n## Validation\n\n${job.checks.map((check) => `- ${check.ok ? "Passed" : "Failed"}: \`${check.command}\``).join("\n")}\n\nGenerated by Stella. Merge and production deployment remain explicit owner actions.`,
    }),
...
<title>Stella Coder · Rotator</title>
<style>
