...
:root{--spmt-bg:#080b14;--spmt-surface:#171321;--spmt-text:#f8fafc;--spmt-accent:#ff8a3d;--spmt-accent-rgb:255,138,61;--spmt-radius:18px;--spmt-glass:.65;--spmt-blur:22px;--spmt-border:.15;--spmt-stars:.7;--spmt-nebula:.8;--spmt-glow:.8;--spmt-density:1;--spmt-motion:1}
...
body.spmt-host-shell:after{z-index:-1;background:radial-gradient(circle at 10% 0%,rgba(var(--spmt-accent-rgb),calc(.25 * var(--spmt-nebula))),transparent 36rem),radial-gradient(circle at 92% 86%,rgba(var(--spmt-accent-rgb),calc(.12 * var(--spmt-nebula))),transparent 34rem),linear-gradient(180deg,color-mix(in srgb,var(--spmt-bg) 88%,#111827),var(--spmt-bg) 72%)}
...
body.spmt-host-shell :is(.panel,.card,.hero,.status,.workspace-card,.surface,.box){background:color-mix(in srgb,var(--spmt-surface) calc(var(--spmt-glass) * 100%),transparent)!important}
body.spmt-host-shell :is(.button.primary,button:not(.secondary):not(.ghost),.primary){background:linear-gradient(135deg,var(--spmt-accent),color-mix(in srgb,var(--spmt-accent) 48%,#ffffff))!important}
body.spmt-host-shell :is(a,.eyebrow,.arrow){--link-accent:var(--spmt-accent)}
body.spmt-host-shell .eyebrow{color:color-mix(in srgb,var(--spmt-accent) 62%,#e5e7eb)!important}
...
.spmt-workspace-host[hidden]{display:none!important}.spmt-workspace-bar{display:flex;align-items:center;gap:8px;padding:9px;border-bottom:1px solid rgba(255,255,255,.1);background:rgba(255,255,255,.025)}.spmt-workspace-tabs{display:flex;gap:7px;flex-wrap:wrap}.spmt-workspace-tabs button,.spmt-workspace-close{display:inline-flex;align-items:center;gap:7px;border:1px solid rgba(255,255,255,.1);border-radius:10px;padding:8px 10px;background:rgba(255,255,255,.035);color:#d4d4d8;font:750 12px Inter,ui-sans-serif,system-ui,sans-serif;cursor:pointer}.spmt-workspace-tabs button.active{border-color:color-mix(in srgb,var(--spmt-accent) 60%,transparent);background:color-mix(in srgb,var(--spmt-accent) 15%,transparent);color:#fff}.spmt-workspace-close{margin-left:auto}.spmt-workspace-frame{width:100%;height:100%;border:0;background:transparent}
...
export function spmtSharedUiScript(appId: string): string {
...
(()=>{
  ...
  function apply(profile){
    ...
    root.style.setProperty('--spmt-bg',p.bg);root.style.setProperty('--spmt-surface',p.surface);root.style.setProperty('--spmt-text',p.text);root.style.setProperty('--spmt-accent',accent);root.style.setProperty('--spmt-accent-rgb',rgb(accent));
  ...
  async function refresh(){try{const r=await fetch('/athena/api/settings',{credentials:'include',cache:'no-store',headers:{accept:'application/json'}});if(!r.ok)return;const data=await r.json();if(data&&data.shared&&data.shared.profile)apply(data.shared.profile)}catch{}}
  function icon(kind){return kind==='settings'?'<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 15.5a3.5 3.5 0 1 0 0-7 3.5 3.5 0 0 0 0 7Z"/><path d="M19.4 15a1.7 1.7 0 0 0 .34 1.88l.06.06-2.86 2.86-.06-.06A1.7 1.7 0 0 0 15 19.4a1.7 1.7 0 0 0-1 .6 1.7 1.7 0 0 0-.4 1.1V21H9.4v-.1A1.7 1.7 0 0 0 8 19.4a1.7 1.7 0 0 0-1.88.34l-.06.06-2.86-2.86.06-.06A1.7 1.7 0 0 0 3.6 15a1.7 1.7 0 0 0-.6-1 1.7 1.7 0 0 0-1.1-.4H1.8V9.4h.1A1.7 1.7 0 0 0 3.6 8a1.7 1.7 0 0 0-.34-1.88L3.2 6.06 6.06 3.2l.06.06A1.7 1.7 0 0 0 8 3.6a1.7 1.7 0 0 0 1-.6 1.7 1.7 0 0 0 .4-1.1V1.8h4.2v.1A1.7 1.7 0 0 0 15 3.6a1.7 1.7 0 0 0 1.88-.34l.06-.06 2.86 2.86-.06.06A1.7 1.7 0 0 0 19.4 8a1.7 1.7 0 0 0 .6 1 1.7 1.7 0 0 0 1.1.4h.1v4.2h-.1A1.7 1.7 0 0 0 19.4 15Z"/></svg>':kind==='overlay'?'<svg viewBox="0 0 24 24" aria-hidden="true"><rect x="3" y="3" width="18" height="18" rx="3"/><path d="M8 8h8v8H8z"/></svg>':'<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 5h16v4H4zM4 11h7v8H4zM13 11h7v8h-7z"/></svg>'}
