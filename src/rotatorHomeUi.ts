...
export async function handleRotatorHomeUiRequest(
  ...
  if (!admin) {
    response.writeHead(302, {
      location: "/mountainview/auth/login?next=%2F",
...
function renderHome(adminName: string): string {
  const cards = [
    ...
    ["MountainView", "/mountainview", "Inspect the MountainView operational surface and existing app-specific administration.", "Legacy admin"],
  ];
  return `<!doctype html>
...
<section class="hero"><div class="eyebrow">SPMT authenticated control plane</div><h1>One front door for Rotator, Athena, and the local LLM.</h1><p class="lead">The inference worker stays private and API-only. All human-facing chat, model selection, worker controls, repair tools, coding jobs, and application operations live here in Rotator under the canonical SPMT admin session.</p><div class="toolbar"><a class="button primary" href="/athena/llm">Open LLM Workbench</a><a class="button" href="/athena">Open Athena Coder</a><a class="button" href="/rotator">Open fleet dashboard</a></div></section>
<section class="status"><div><span class="dot"></span> Signed in as <strong>${escapeHtml(adminName)}</strong></div><div>Worker UI: disabled · Rotator UI: authoritative</div></section>
