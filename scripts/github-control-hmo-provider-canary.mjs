...
(async()=>{
  ...
  const post=async(path,body,timeout=25000)=>step('worker '+path,async()=>{
    ...
    if(!response.ok||payload.success===false)throw new Error('failed ('+response.status+')');
    return payload;
  });
  try{
    ...
    if(!candidate)throw new Error('channel-selection: no empty canary/test/sandbox/dev/bot Discord voice channel is available');
    candidateKind=candidate.kind;
    roomId='apollo-canary-'+Date.now();
    startAttempted=true;
    await post('/voice-bridge',{action:'start',roomId,guildId:candidate.guildId,voiceChannelId:candidate.voiceChannelId,audioProfile:'balanced'},30000);
    started=true;
...
async function main() {
  try {
    ...
    const result = await fly(['ssh', 'console', '--app', APP, '--machine', String(active[0].id), '--command', command, '--quiet'], { timeout: 150000 });
