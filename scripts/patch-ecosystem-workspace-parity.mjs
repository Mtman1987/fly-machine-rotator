...
await patch('mobile/App.tsx', (source) => {
  ...
  if (source.includes(targetBlock) && !source.includes('Canonical workspace control center')) {
    const workspaceBlock = `          {tab === "workspace" && (\n            <View style={styles.panel}>\n              <Text style={styles.label}>Canonical workspace control center</Text>\n              <Text style={styles.note}>The same account Worktray and shared surfaces are resolved by SPMT, so MountainView does not keep its own editor URLs or shared state.</Text>\n              <View style={styles.workspaceSummary}>\n                <Text style={styles.workspaceTheme}>Preset: {String(workspace?.profile?.appearance?.themeId || "Sign in to SPMT")}</Text>\n                <Text style={styles.note}>{workspace?.profile ? "Workspace profile linked to this MountainView session." : "Reconnect SPMT to restore the canonical Workspace surface."}</Text>\n              </View>\n              {workspace?.canonical?.surfaceUrls?.worktray ? (\n                <WebView\n                  source={{ uri: String(workspace.canonical.surfaceUrls.worktray) }}\n                  style={styles.workspaceWebView}\n                  sharedCookiesEnabled\n                  thirdPartyCookiesEnabled\n                  originWhitelist={["https://*"]}\n                />\n              ) : (\n                <Text style={styles.note}>Canonical SPMT Worktray is unavailable for this session.</Text>\n              )}\n            </View>\n          )}\n\n`;
