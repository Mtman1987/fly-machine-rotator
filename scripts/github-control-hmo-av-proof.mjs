...
(async()=>{
  ...
  const out={ok:false,fixtureVideoId:videoId,sourceInputs:1,encoderProcesses:1,intermediateYoutubeHls:false,liveKitMedia:false};
  let tmp='';
  try{
    ...
    const raw=run('yt-dlp',['--ignore-config','--js-runtimes','node','--dump-single-json','--no-playlist','--no-warnings','--',watch],120000);
    const body=JSON.parse(raw);
    const formats=Array.isArray(body.formats)?body.formats:[];
    const combined=[...formats].reverse().find(f=>typeof f?.url==='string'&&f.vcodec&&f.vcodec!=='none'&&f.acodec&&f.acodec!=='none');
    out.totalFormats=formats.length;
    out.combinedFormats=formats.filter(f=>typeof f?.url==='string'&&f.vcodec&&f.vcodec!=='none'&&f.acodec&&f.acodec!=='none').length;
    out.videoOnlyFormats=formats.filter(f=>typeof f?.url==='string'&&f.vcodec&&f.vcodec!=='none'&&(!f.acodec||f.acodec==='none')).length;
    out.audioOnlyFormats=formats.filter(f=>typeof f?.url==='string'&&f.acodec&&f.acodec!=='none'&&(!f.vcodec||f.vcodec==='none')).length;
    if(!combined)throw new Error('Apollo selection rule found no single muxed video+audio format');
    out.selectedFormatId=String(combined.format_id||'').slice(0,40);
    out.selectedHeight=Number.isFinite(Number(combined.height))?Number(combined.height):null;
    out.selectedVideoCodec=String(combined.vcodec||'').slice(0,80);
    out.selectedAudioCodec=String(combined.acodec||'').slice(0,80);
    const sourceProbe=JSON.parse(run('ffprobe',['-v','error','-rw_timeout','15000000','-show_streams','-of','json',combined.url],30000));
    const sourceStreams=Array.isArray(sourceProbe.streams)?sourceProbe.streams:[];
    out.sourceVideoStreams=sourceStreams.filter(s=>s.codec_type==='video').length;
    out.sourceAudioStreams=sourceStreams.filter(s=>s.codec_type==='audio').length;
    if(out.sourceVideoStreams<1||out.sourceAudioStreams<1)throw new Error('ffprobe did not confirm both video and audio on the selected single URL');
    tmp=fs.mkdtempSync(path.join(os.tmpdir(),'hmo-av-proof-'));
    const index=path.join(tmp,'index.m3u8');
    run('ffmpeg',['-hide_banner','-loglevel','error','-nostdin','-y','-threads','2','-t','8','-i',combined.url,'-map','0:v:0','-map','0:a:0','-c:v','libx264','-preset','veryfast','-pix_fmt','yuv420p','-c:a','aac','-ac','2','-b:a','128k','-f','hls','-hls_time','2','-hls_list_size','0','-hls_segment_filename',path.join(tmp,'seg%03d.ts'),index],90000);
    ...
    if(!out.hlsManifest||segments.length<1)throw new Error('single encoder did not produce HLS output');
    ...
    out.ok=true;
    out.stage='complete';
  }catch(e){out.stage=out.combinedFormats===0?'resolve':'probe-or-encode';out.error=safe(e)}finally{if(tmp){try{fs.rmSync(tmp,{recursive:true,force:true})}catch{}}}
