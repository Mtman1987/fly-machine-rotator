...
describe("incident classifier", () => {
  ...
  it("groups the July 24 outbound outage and preserves the DSH XP scope defect", () => {
    ...
    expect(classifyIncident(event("streamweaver-new", "dsh-admin", "[Next.js ERROR] [DiscordStreamHub] Admin access check failed: The operation was aborted due to timeout"))).toMatchObject({ key: "streamweaver-new:dsh-admin-access-timeout", disposition: "code", autoFixEligible: true });
