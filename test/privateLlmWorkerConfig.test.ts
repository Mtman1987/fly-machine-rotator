...
describe("current SPMT Qwen worker configuration", () => {
  it("keeps the existing spmt-qwen3-4b worker on Fly private networking", async () => {
    ...
    expect(flyToml).toContain('LLAMA_ARG_ALIAS = "spmt-qwen3-4b"');
    expect(flyToml).toContain('Qwen/Qwen3-4B-GGUF:Q4_K_M');
  });
  ...
  it("cleans stale auth and smoke-tests the real keyless chat endpoint on deploy", async () => {
